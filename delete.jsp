...
<%
    ...
    if (taskName != null && !taskName.isEmpty()) {
        ...
        try {
            ...
            conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/task", "root", "8910@Sai");

            conn.setAutoCommit(false);

            String insertSQL = "INSERT INTO delete_tas (user_name, task_name, discription, priority, date) VALUES (?, ?, ?, ?, ?)";
            ...
            if (insertResult > 0) {
                String deleteSQL = "DELETE FROM tasks WHERE user_name = ? AND task_name = ?";
                ...
                if (deleteResult > 0) {
                    String updateAnalyticsSQL = "UPDATE analytics SET deleted_tasks = deleted_tasks + 1 WHERE user_name = ?";
                    ...
                    if (analyticsUpdated > 0) {
                        conn.commit();
                        out.println("<script>alert('Task moved to trash successfully!'); window.location='ii.jsp';</script>");
                    } else {
                        conn.rollback();
                    ...
                    conn.rollback();
                    out.println("<p style='color:orange;'>Task not found, but inserted into trash!</p>");
                }
            } else {
                out.println("<p style='color:red;'>Failed to insert task into trash!</p>");
            }
        } catch (Exception e) {
            e.printStackTrace();
